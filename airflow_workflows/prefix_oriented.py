...
from localutils import add_to_db, get_file_list, print_db_stats, setup_daos
...
def transform_objects(**kwargs):
    ...
    def process(x): return local_process_file(x['Name'])
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=None) as executor:
        for obj, results in zip(pfl.get_list(), executor.map(process, pfl.get_list())):
            ...
            for it in results:
                add_to_db(station_dao, series_dao, mes_dao,
                          station=it[0], measurement=it[1])
...
dag = DAG('prefix-oriented', default_args=default_args,
